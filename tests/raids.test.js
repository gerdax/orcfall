...
test('night creates five raiders per village only once, and next night creates a new wave',()=>{
 ...
 p.night=1;p.sync(w,chunks);assert.equal(raiders().length,5);
 ...
 p.sync(w,chunks);assert.equal(raiders().length,5);
 p.damageOrc(a,4);p.damageOrc(a,4);p.damageOrc(a,4);
 p.sync(w,[]);p.sync(w,chunks);assert.equal(raiders().length,4);
 p.night=2;p.sync(w,chunks);assert.equal(raiders().length,9);
});
