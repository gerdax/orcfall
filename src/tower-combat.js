...
export class TowerCombat{
 ...
 update(dt,objects,actors,time,damage){
  this.arrows=this.arrows.filter(a=>{
   ...
   if(target?.type==='orc'&&Math.hypot(target.x-a.tx,target.y-a.ty)<18)damage(target,time);
   return false;
  });
  ...
  for(const tower of towers){
   ...
   for(const a of actors.values())if(a.type==='orc'&&a.hp>0){const d=Math.hypot(tower.x-a.x,tower.y-a.y);if(d<nearest){nearest=d;target=a}}
   if(!target)continue;
