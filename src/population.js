...
export class Population{
 ...
 sync(world,chunks){
  ...
  for(const [id,a] of this.actors)if(!present.has(a.chunk)){this.actors.delete(id)}
 ...
 update(dt,hero,objects,time,hurt){
  ...
  for(const a of this.actors.values()){
   ...
   if(Math.hypot(dx,dy)>3){a.dir=Math.abs(dx)>Math.abs(dy)?dx>0?'right':'left':dy>0?'down':'up';a.moving=moveActor(a,dx,dy,dt,objects,null,null,hostile?36:a.type==='human'?13:17)}
  }
 }
