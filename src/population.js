...
export class Population{
 constructor(){this.guards=new GuardCombat();this.raidWaves=new Set();this.raidActors=new Map();this.night=-1;this.towers=new TowerCombat();this.traders=new Map();this.corpses=[];this.actors=new Map();this.defeated=new Set();this.loaded=new Set()}
 sync(world,chunks){
  ...
  for(const hut of obstacles.filter(o=>o.fishing)){const id=`fisher:${hut.x}:${hut.y}`;if(!this.actors.has(id)){const a=makeFisherman(world,hut,obstacles);if(a)this.actors.set(id,a)}}
  const towns=new Map();
  ...
  for(const c of chunks){const key=`${c.cx}:${c.cy}`;if(this.loaded.has(key))continue;this.loaded.add(key);
   for(const a of c.campActors??[])if(!this.defeated.has(a.id))this.actors.set(a.id,{...a});
   ...
   for(let i=0;i<16;i++){
    ...
    this.actors.set(id,{id,chunk:key,type:human?'human':hash(c.cx,c.cy,world.seed+905)<.35?'goblin':'orc',x,y,homeX:x,homeY:y,hp:human||hash(c.cx,c.cy,world.seed+905)>=.35?3:2,dir:'down',moving:false,timer:0,cooldown:0,windup:0,flash:0,variant:Math.floor(hash(c.cx,c.cy,world.seed+904)*3)});break;
   }
  }
 }
 update(dt,hero,objects,time,hurt){
  ...
  this.guards.updateProjectiles(dt,this.actors,objects,time,(a,t)=>this.damageOrc(a,t));
  for(const a of this.actors.values()){
   if(a.type==='fisherman')continue;
   if(a.type==='merchant'){updateMerchant(a,dt);continue}
   if(isGuard(a)&&this.guards.update(a,dt,this.actors,objects,time,(o,t)=>this.damageOrc(o,t)))continue;
   a.cooldown=Math.max(0,a.cooldown-dt);a.timer-=dt;
   const d=Math.hypot(hero.x-a.x,hero.y-a.y),hostile=['orc','goblin'].includes(a.type)&&d<110&&(a.raid||Math.hypot(hero.x-a.homeX,hero.y-a.homeY)<175)&&clearLine(a,hero,objects);
   a.moving=false;
   if(a.windup>0){a.windup-=dt;if(a.windup<=0){if(d<23&&clearLine(a,hero,objects))hurt(a.boss?2:1);a.cooldown=1.2}continue}
...
export function drawPerson(ctx,a,time){if(a.type==='fisherman'){drawFisherman(ctx,a,time);return}if(a.boss){drawChief(ctx,a,time);return}if(a.type==='merchant'){drawMerchant(ctx,a,time);return}if(['knight','archer','spearman'].includes(a.type)){drawGuard(ctx,a,time);return}const orc=['orc','goblin'].includes(a.type),rows=a.type==='goblin'?GOBLIN:orc?ORC:HUMAN,palette={o:'#17201b',h:'#593824',H:'#8f663c',s:'#c99568',S:'#e8b982',c:['#92764d','#806280','#668984'][a.variant],b:'#3a3024',g:'#486338',G:'#79914b',Y:'#e3c77b',W:'#ded3a4',t:'#483d2c',T:'#776044'};const x=Math.round(a.x)-8,y=Math.round(a.y)-8;ctx.fillStyle='#293c2d';ctx.fillRect(x+2,y+13,13,3);for(let r=0;r<16;r++)for(let c=0;c<16;c++){let p=rows[r][c];if(!palette[p])continue;if(a.dir==='up'&&r>3&&r<7)p=orc?'g':'h';ctx.fillStyle=time<a.flash?'#eee1bd':palette[p];const step=r>11&&a.moving?(Math.floor(time*7)%2?1:-1)*(c<8?1:-1):0;ctx.fillRect(x+c,y+r+step,1,1)}if(orc){ctx.fillStyle='#6e5840';ctx.fillRect(x+15,y+6,1,7);ctx.fillStyle='#a7ada0';ctx.fillRect(x+14,y+5,4,4);ctx.fillStyle='#182019';ctx.fillRect(x+2,y-5,13,3);ctx.fillStyle=a.windup>0?'#efb853':'#b44535';ctx.fillRect(x+3,y-4,Math.ceil(11*a.hp/(a.type==='goblin'?2:3)),1);if(a.windup>0){ctx.fillStyle='#efb853';ctx.fillRect(x+7,y-12,2,4);ctx.fillRect(x+7,y-7,2,1)}}}
