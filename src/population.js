...
export class Population{
 ...
 sync(world,chunks){
  ...
  for(const [id,a] of this.actors)if(a.type!=='knight'&&!present.has(a.chunk)){this.actors.delete(id)}
...
export function drawPerson(ctx,a,time){if(a.type==='knight'){drawKnight(ctx,a,time);return}const orc=a.type==='orc',rows=orc?ORC:HUMAN,palette={o:'#17201b',h:'#593824',H:'#8f663c',s:'#c99568',S:'#e8b982',c:['#92764d','#806280','#668984'][a.variant],b:'#3a3024',g:'#486338',G:'#79914b',Y:'#e3c77b',W:'#ded3a4',t:'#483d2c',T:'#776044'};const x=Math.round(a.x)-8,y=Math.round(a.y)-8;ctx.fillStyle='#293c2d';ctx.fillRect(x+2,y+13,13,3);for(let r=0;r<16;r++)for(let c=0;c<16;c++){let p=rows[r][c];if(!palette[p])continue;if(a.dir==='up'&&r>3&&r<7)p=orc?'g':'h';ctx.fillStyle=time<a.flash?'#eee1bd':palette[p];const step=r>11&&a.moving?(Math.floor(time*7)%2?1:-1)*(c<8?1:-1):0;ctx.fillRect(x+c,y+r+step,1,1)}if(orc){ctx.fillStyle='#6e5840';ctx.fillRect(x+15,y+6,1,7);ctx.fillStyle='#a7ada0';ctx.fillRect(x+14,y+5,4,4);ctx.fillStyle='#182019';ctx.fillRect(x+2,y-5,13,3);ctx.fillStyle=a.windup>0?'#efb853':'#b44535';ctx.fillRect(x+3,y-4,Math.ceil(11*a.hp/3),1);if(a.windup>0){ctx.fillStyle='#efb853';ctx.fillRect(x+7,y-12,2,4);ctx.fillRect(x+7,y-7,2,1)}}}
