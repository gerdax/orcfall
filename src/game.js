import {awardXP,xpNeeded} from './progression.js';
import {daylight,drawNight} from './day-night.js';
import {drawFishingGear} from './countryside.js';
import {healAtWell} from './healing.js?v=0.11.0';
import {Population,drawPerson,drawCorpse} from './population.js?v=0.13.0';
import {drawCastle} from './castles.js';
import {drawScenery} from './scenery.js';
import {drawSettlement,isSettlementObject} from './settlements.js?v=0.13.0';
import {World,CHUNK_SIZE,DEFAULT_SEED} from './world.js?v=0.13.0';
...
function gainXP(amount){if(awardXP(hero,amount)){burst(hero.x,hero.y,'#eed587',24);notify(`Awans! Poziom ${hero.level} · zdrowie ${hero.maxHp} · miecz ${hero.damage}`,4);beep(880,.2)}updateUI()}
function updateUI(){
 $('level').textContent=`Lv. ${hero.level}`;$('xp-fill').style.width=`${hero.xp/xpNeeded(hero.level)*100}%`;$('xp-label').textContent=`${hero.xp} / ${xpNeeded(hero.level)} PD`;
  $('health-fill').style.width=`${hero.hp/hero.maxHp*100}%`;$('health-fill').parentElement?.setAttribute('aria-label',`Zdrowie ${hero.hp} z ${hero.maxHp}`);
...
function attack(){if(paused||time<attackCooldown)return;attackUntil=time+.23;attackCooldown=time+.4;beep(175);const result=population.strike(hero,angle(),nearbyObjects,time);if(result.hits){beep(95);notify(result.kills?'Ork pokonany.':'Celny cios w orka!')}if(result.xp)gainXP(result.xp);for(const d of dummies){const distance=Math.hypot(d.x-hero.x,d.y-hero.y),a=Math.atan2(d.y-hero.y,d.x-hero.x),diff=Math.atan2(Math.sin(a-angle()),Math.cos(a-angle()));if(distance<29&&(Math.abs(diff)<1.25||distance<12)){d.flash=time+.2;hits++;burst(d.x,d.y,'#dfc68b');notify('Celny cios! Kukła wytrzyma kolejny.');beep(110);updateUI()}}}
...
$('pause').onclick=()=>setPause(!paused);$('resume').onclick=()=>{if(hero.hp<=0){hero.x=hero.y=0;hero.hp=hero.maxHp;hero.hurtUntil=time+2;refreshWorld();updateUI()}setPause(false)};$('reset').onclick=()=>reset();$('new-world').onclick=()=>reset(Math.floor(Math.random()*4294967296));$('sound').onclick=()=>{sound=!sound;$('sound').textContent=`Dźwięk: ${sound?'wł.':'wył.'}`;$('sound').setAttribute('aria-pressed',sound);beep(440)};
...
function drawMinimap(){
 ...
 for(const t of world.townsIn(left,top,left+640,top+640))for(const o of t.objects){
  if(!['house','inn','stable','castle-keep','castle-tower','watchtower','chief-tower'].includes(o.type))continue;
